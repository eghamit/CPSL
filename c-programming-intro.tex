		...
		\begin{columns}[T]
			...
			\begin{column}{0.42\textwidth}
				...
				\begin{tikzpicture}[scale=0.7, transform shape,
					every node/.style={font=\scriptsize}, node distance=0.5cm]
					\node[startstop] (start) {Start};
					\node[io, below=of start, fill=cOrange!20] (fetch) {Fetch};
					\node[process, below=of fetch, fill=blue!10, minimum width=2cm, text width=2cm] (decode) {Decode};
					\node[process, below=of decode, fill=cGreen!14, minimum width=2cm, text width=2cm] (exec) {Execute};
					\node[decision, below=0.65cm of exec] (halt) {HALT?};
					\node[startstop, below=0.65cm of halt] (stop) {Stop};
					\draw[flowarrow] (start) -- (fetch);
					\draw[flowarrow] (fetch) -- (decode);
					\draw[flowarrow] (decode) -- (exec);
					\draw[flowarrow] (exec) -- (halt);
					\draw[flowarrow] (halt) -- node[right,font=\tiny]{yes} (stop);
					\draw[flowarrow] (halt.east) -- ++(0.7,0)
					node[midway,above,font=\tiny]{no} |- (fetch.east);
					\node[font=\tiny, cAccent, right=1.4cm of decode] {PC advances};
				\end{tikzpicture}
			\end{column}
		\end{columns}
	\end{frame}
